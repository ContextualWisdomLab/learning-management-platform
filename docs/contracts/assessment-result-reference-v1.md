...
The route stores `evidence_kind=assessment`, the authority/reference/digest/version, the external result status, idempotency key, registration binding, and source observation time. Repeating the same idempotency key with the same source request returns the original reference; reusing it for a different registration or source is a conflict. Rust completion evaluation accepts assessment evidence only when its external status is `passed`; `failed` and `inconclusive` remain immutable evidence but cannot produce a completed decision.
...
The idempotency key is scoped to the tenant and assessment authority. A retry must reuse it; a new external result must use a new key.
