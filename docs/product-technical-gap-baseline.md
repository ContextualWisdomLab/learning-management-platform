...
| PR [#4](https://github.com/ContextualWisdomLab/learning-management-platform/pull/4), current head `9de3bcf` | Adds the tenant-safe learner kernel and API smoke path; its CI now uses a `NOSUPERUSER NOBYPASSRLS` application role, covers customer/self-sponsored and multi-tenant affiliation cases, and rehearses disposable migration rollback/reapply. | It remains open with independent review and current protected checks required; it is not merged product evidence. |
