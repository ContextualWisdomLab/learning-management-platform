...
| PR [#1](https://github.com/ContextualWisdomLab/learning-management-platform/pull/1), exact head `cdda6ece5c4624e45c3166241bfa14fead3de5fc` | Adds governance, architecture, data-model, ADR, standards, and quality-workflow documents. It remains open with `REVIEW_REQUIRED`; current checks are queued. | The bootstrap is not merged evidence and must not be described as implemented LMS behavior. |
