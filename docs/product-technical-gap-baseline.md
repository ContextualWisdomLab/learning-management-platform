...
| PR [#1](https://github.com/ContextualWisdomLab/learning-management-platform/pull/1), exact head `61f9d8c` | Adds governance, architecture, data-model, ADR, standards, exact-head quality validation, explicit evidence-metadata allowlisting, and registration-to-decision cardinality. It remains open with review and checks pending. | The bootstrap is not merged evidence and must not be described as implemented LMS behavior. |
...
| PR [#14](https://github.com/ContextualWisdomLab/learning-management-platform/pull/14), implementation head `0fd8490` | Propagates optional `X-Correlation-ID` UUIDs into the PR13 audit events and keeps generated IDs for callers without the header. | It remains open with checks/review pending; it is not merged product evidence. |
| PR [#15](https://github.com/ContextualWisdomLab/learning-management-platform/pull/15), implementation head `c667fc1` | Adds bounded tenant-scoped audit-event export that returns provenance metadata without source payloads. | It remains open with checks/review pending; it is not merged product evidence. |
| PR [#16](https://github.com/ContextualWisdomLab/learning-management-platform/pull/16), implementation head `bb6ec38` | Adds exact-migration operational catalog checks and an ADR/runbook for RLS, indexing, retention, and rollback evidence boundaries. | It remains open with checks/review pending; it is not merged product evidence. |
| PR [#17](https://github.com/ContextualWisdomLab/learning-management-platform/pull/17), branch `feat/completion-correction` (re-fetch exact live head before merge) | Adds an immutable completion correction request with same-registration predecessor validation, explicit supersession, a distinct audit action, and explicit PRD/TRD/UML/ADR traceability. | It remains open with checks/review pending; it is not merged product evidence. |
| PR [#18](https://github.com/ContextualWisdomLab/learning-management-platform/pull/18), branch `feat/postgresql-recovery-evidence` (re-fetch exact live head before merge) | Adds a disposable PostgreSQL backup/restore rehearsal with measured tenant skew, forced-RLS preservation, and a forward audit-correlation index migration. | It is stacked on PR #17 and remains open with checks/review pending; it is not merged product evidence. |
| PR [#19](https://github.com/ContextualWisdomLab/learning-management-platform/pull/19), branch `feat/audit-export-cursor` (re-fetch exact live head before merge) | Adds stable keyset pagination to the tenant audit export with partial-cursor rejection and exact-head API smoke coverage. | It is stacked on PR #18 and remains open with checks/review pending; it is not merged product evidence. |
| PR [#20](https://github.com/ContextualWisdomLab/learning-management-platform/pull/20), branch `feat/audit-export-receipt` (re-fetch exact live head before merge) | Adds backward-compatible audit-export receipt headers for page count, ordered digest, and the next keyset cursor, with exact-head recomputation smoke coverage. | It is stacked on PR #19 and remains open with checks/review pending; it is not merged product evidence. |
...
| G-04 | P0 | No time-aware affiliation or correction model | PR #6 provides effective-dated affiliations; PR #17 creates a new immutable completion decision with an explicit predecessor relation | Effective-dated and replay/correction tests pass | Add transaction-time metadata and correction policy governance |
...
| G-06 | P0 | No deterministic completion/evidence engine | PR #8 evaluates versioned policy/evidence in Rust; PR #11 rejects failed or inconclusive assessment evidence; PR #12 binds evidence to registration; PR #17 adds validated supersession | Replay produces the same decision from policy/evidence versions on a real database | Add provider contract tests and transaction-time replay metadata |
| G-07 | P1 | No provenance, audit, or operational evidence | PR #13 adds tenant-isolated append-only audit events; PR #14 propagates optional caller correlation UUIDs; PR #15 adds bounded tenant-scoped audit export; PR #19 adds stable keyset pagination for large histories; PR #20 adds caller-retainable page receipts | Correlation-linked audit and decision history with export, pagination, operational receipts, retention, and incident evidence | Add retention and incident evidence |
| G-08 | P1 | No PostgreSQL hot-partition, retention, or production rollback evidence | PRs #4–#9 provide 3NF tables, composite tenant FKs, RLS, and embedded migrations; PR #16 verifies exact catalog RLS/index invariants; PR #18's recovery rehearsal preserves a 900/100 tenant distribution through dump/restore and applies the forward correlation-index migration | Production-shaped migration, RLS, load, retention, and rollback evidence | Measure load, vacuum, query plans, and tenant skew before choosing partitioning, retention, or rollback procedures |
...
3. Issue #3: stack the external-learner vertical on that foundation; PR #5 covers registration, PR #6 affiliation, PR #7 launch/progress, PR #8 completion persistence, PR #9 credential issuance, PR #10 credential revocation, PR #11 assessment-result reference handoff, PR #12 retry/binding safety, PR #13 audit/provenance events, PR #14 correlation propagation, PR #15 audit export, PR #16 operational evidence, PR #17 completion correction, PR #18 disposable PostgreSQL recovery evidence, PR #19 stable audit-export pagination, and PR #20 export operational receipts.
