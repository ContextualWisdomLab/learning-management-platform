...
| PR #11 (next stack, local branch) | Adds the versioned `assessment_result_reference/v1` boundary, external pass/fail status, and Rust rejection of non-passed assessment evidence. | It is not yet a GitHub PR or merged product evidence. |
...
| G-03 | P0 | No completed external learner journey | PR #5 implements offering through registration; PR #7 adds launch/progress; PR #8 adds completion decision persistence; PR #9 adds credential reference issuance; PR #10 adds revocation; PR #11 adds assessment result reference handoff | Non-employee journey passes browser/API E2E through completion and credential lifecycle | Add assessment execution/scoring, browser E2E, and released contracts |
...
| G-05 | P0 | No versioned external contracts | PR #11 adds the local `assessment_result_reference/v1` JSON schema, route, and exact-head smoke path; no released provider client exists | Schemas, clients, provider-consumer contract tests, idempotent adapters | Add integration package, client, and outbox |
| G-06 | P0 | No deterministic completion/evidence engine | PR #8 evaluates versioned policy/evidence in Rust; PR #11 rejects failed or inconclusive assessment evidence | Replay produces the same decision from policy/evidence versions on a real database | Add correction/supersession and provider contract tests |
...
3. Issue #3: stack the external-learner vertical on that foundation; PR #5 covers registration, PR #6 affiliation, PR #7 launch/progress, PR #8 completion persistence, PR #9 credential issuance, PR #10 credential revocation, and PR #11 assessment-result reference handoff.
